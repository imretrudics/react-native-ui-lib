...
const HintTestComponent = ({
  ...
  return <Hint visible={showHint} message={'Hint message to hint things'} position={Hint.positions.TOP} useSideTip key={'1'} targetFrame={{
    x: 1,
    y: 1,
    height: 1,
    width: 1
  }} onBackgroundPress={() => {}} color={color} removePaddings enableShadow testID={'Hint'} />;
};
describe('Hint Screen component test', () => {
  ...
  it('Test Hint component background color', async () => {
    ...
    const WhiteHintDriver = new HintDriver({
      component: WhiteHint,
      testID: 'Hint'
    });
